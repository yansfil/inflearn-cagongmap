...
import CafeSeoList from "../components/CafeSeoList";
import { getCafes } from "../lib/cafes";
import { cafeItemList } from "../lib/structuredData";

export default async function Home() {
  const cafes = await getCafes();
  const kakaoKey = process.env.NEXT_PUBLIC_KAKAO_MAP_KEY;

  // 카페 목록 구조화 데이터(ItemList<CafeOrCoffeeShop>) — 지도 키 유무와 무관하게
  // 크롤러가 카페 정보를 읽을 수 있도록 항상 렌더한다.
  const itemListJson =
    cafes.length > 0 ? JSON.stringify(cafeItemList(cafes)) : null;
  ...
  return (
    <>
      ...
      <CafeSeoList cafes={cafes} />
      <KakaoMap cafes={cafes} appKey={kakaoKey} />
    </>
  );
}
